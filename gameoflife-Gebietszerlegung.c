...
void game(int w, int h, int timesteps, int myrank) {
...
///< read height
  ...
  int coords[1];
  MPI_Cart_coords(cart_comm, myrank, dims[0], coords);
  //printf("%d\n", myrank);
  int links,rechts;
  MPI_Cart_shift(cart_comm, 0, 1, &links, &rechts);
  if(myrank == 0)
     printf("Process %d: keinen linken Nachbar, rechts: %d, coords: %d, Gebietsgröße: %d\n",myrank, rechts, coords[0], h*w/dims[0]);
   else if(myrank == (dims[0] -1))
      printf("Process %d: links: %d, keinen rechten Nachbar, coords: %d, Gebietsgröße: %d\n",myrank, links, coords[0], h*w/dims[0]);
    else
      printf("Process %d: links: %d,rechts: %d, coords: %d, Gebietsgröße: %d\n",myrank, links, rechts, coords[0], h*w/dims[0]);

  game(w, h/dims[0], timesteps,myrank);
  MPI_Finalize();
}
